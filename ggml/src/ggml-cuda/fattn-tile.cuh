...
// Keep the initial direct-quantized tile contract deliberately narrow. All other
// devices, cache types, shapes, and request layouts retain the existing F16
// materialization path. This predicate is shared by allocation and launch so a
// graph can never reserve one layout and execute the other.
static inline bool ggml_cuda_fattn_tile_use_gfx1030_q8_0(const int device, const ggml_tensor * dst) {
#if defined(GGML_USE_HIP)
    if (!ggml_cuda_rdna2_feature_enabled("GGML_HIP_GFX1030_FATTN_Q8_TILE") ||
            !GGML_CUDA_CC_IS_RDNA2(ggml_cuda_info().devices[device].cc)) {
        return false;
    }
    ...
    return Q->type == GGML_TYPE_F32 && K->type == GGML_TYPE_Q8_0 && V->type == GGML_TYPE_Q8_0 &&
        Q->ne[0] == 256 && K->ne[0] == 256 && V->ne[0] == 256 &&
        Q->ne[1] >= 3 && Q->ne[1] <= 8 &&
        Q->ne[2] == 6*K->ne[2] && V->ne[2] == K->ne[2] &&
        Q->ne[3] == 1 && K->ne[3] == 1 && V->ne[3] == 1 &&
        K->ne[1] == V->ne[1] && K->ne[1] % FATTN_KQ_STRIDE == 0 &&
        mask != nullptr && max_bias == 0.0f;
#else
    GGML_UNUSED(device, dst);
    return false;
#endif
}
...
// Dequantize quantized K/V directly into the shared tile. This produces the
// same half values as the bulk Q8_0-to-F16 converter, but avoids materializing
// and rereading a full-cache F16 workspace. The tile is shared by all grouped Q
// heads in the block, so dequantization does not scale with the GQA ratio.
...
static __device__ __forceinline__ void flash_attn_tile_load_tile_q(
    ...
    static_assert(type_KV == GGML_TYPE_Q8_0, "only the certified Q8_0 tile path is instantiated");
    constexpr int ne_call  = 8;
    constexpr int epc      = 8;
...
#pragma unroll
    for (int idx0 = 0; idx0 < I*cpr; idx0 += nthreads) {
        ...
        if constexpr (std::is_same_v<T, half2>) {
            ...
            __align__(16) half2 tmp[epc/2];
#pragma unroll
            for (int k = 0; k < ncalls; ++k) {
                if (!oob_check || i < i_sup) {
                    dequantize_KV(KV + (size_t) i*stride_KV_bytes, tmp + k*(ne_call/2), elem0 + j + k*ne_call);
                } else {
#pragma unroll
                    for (int l = 0; l < ne_call/2; ++l) {
                        tmp[k*(ne_call/2) + l] = make_half2(0.0f, 0.0f);
                    }
                }
            }
#pragma unroll
            for (int l = 0; l < epc/2; ++l) {
                tile_KV[i*(J/2 + J_padding) + j/2 + l] = tmp[l];
            }
        } else {
            constexpr dequantize_V_t dequantize_KV = get_dequantize_V<type_KV, float, ne_call>();
            __align__(16) float tmp[epc];
#pragma unroll
            for (int k = 0; k < ncalls; ++k) {
                if (!oob_check || i < i_sup) {
                    dequantize_KV(KV + (size_t) i*stride_KV_bytes, tmp + k*ne_call, elem0 + j + k*ne_call);
                } else {
#pragma unroll
                    for (int l = 0; l < ne_call; ++l) {
                        tmp[k*ne_call + l] = 0.0f;
                    }
                }
            }
#pragma unroll
            for (int l = 0; l < epc; ++l) {
                tile_KV[i*(J + J_padding) + j + l] = tmp[l];
            }
        }
    }
}
...
template <int warp_size, int nwarps, int ncols1, int ncols2, int DKQ, int nbatch_fa, int nbatch_K,
    bool use_logit_softcap, bool oob_check, bool use_gfx1030_native, ggml_type type_K, typename T_vec_dot>
static __device__ __forceinline__ void flash_attn_tile_iter_KQ(
    ...
    if constexpr (type_K == GGML_TYPE_F16) {
        ...
        flash_attn_tile_load_tile_q<type_K, warp_size, nwarps, nbatch_fa, nbatch_K, cpy_ne, oob_check>
            (K_c + int64_t(k_VKQ_0)*stride_K_bytes, KV_tmp, stride_K_bytes, k_KQ_0, k_VKQ_sup);
    }
    __syncthreads();
...
#pragma unroll
                for (int k = 0; k < cpy_ne; ++k) {
                    ggml_cuda_mad_f16_gfx1030<use_gfx1030_native>(KQ_acc[i_KQ_0/(np*warp_size)*cpw + jc0],
                        K_k[i_KQ_0/(np*warp_size)][k], Q_k[jc0][k]);
                }
            }
        }
    }
...
template <int warp_size, int nwarps, int ncols1, int ncols2, int DKQ, int DV, int nbatch_fa, int nbatch_K,
    bool use_logit_softcap, bool oob_check, bool use_gfx1030_native, ggml_type type_K, ggml_type type_V,
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < DKQ - nbatch_K_last; k_KQ_0 += nbatch_K) {
        flash_attn_tile_iter_KQ<warp_size, nwarps, ncols1, ncols2, DKQ, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, use_gfx1030_native, type_K>(
            Q_tmp, K_h2, K_c, KV_tmp, stride_K2, stride_K_bytes, k_VKQ_0, k_VKQ_sup, k_KQ_0, KQ_acc);
    }
    if constexpr (nbatch_K_last > 0) {
        constexpr int k_KQ_0 = DKQ - nbatch_K_last;
        flash_attn_tile_iter_KQ<warp_size, nwarps, ncols1, ncols2, DKQ, nbatch_fa, nbatch_K_last, use_logit_softcap, oob_check, use_gfx1030_native, type_K>(
            Q_tmp, K_h2, K_c, KV_tmp, stride_K2, stride_K_bytes, k_VKQ_0, k_VKQ_sup, k_KQ_0, KQ_acc);
    }
...
#pragma unroll
        ...
        KQ_max_new[jc0] = ggml_cuda_warp_reduce_max_gfx1030<use_gfx1030_native, warp_size>(KQ_max_new[jc0]);
    }
    ...
    if constexpr (np == 1) {
        ...
        KQ_max_new[0] = ggml_cuda_warp_reduce_max_gfx1030<use_gfx1030_native, np>(KQ_max_new[0]);
    }
...
#pragma unroll
    for (int k0 = 0; k0 < nbatch_fa; k0 += nbatch_V) {
        if constexpr (type_V == GGML_TYPE_F16) {
            ...
            flash_attn_tile_load_tile_q<type_V, warp_size, nwarps, nbatch_V, DV, 0, oob_check>
                (V_c + int64_t(k_VKQ_0 + k0)*stride_V_bytes, KV_tmp, stride_V_bytes, 0, k_VKQ_sup - k0);
        }
        __syncthreads();
...
template<int DKQ, int DV, int ncols1, int ncols2, bool use_logit_softcap, bool use_gfx1030_native,
...
#pragma unroll
    ...
    if (ncols2 == 1) {
        ...
        while (k_VKQ_0 < k_VKQ_max - nbatch_fa) {
            ...
            flash_attn_tile_iter<warp_size, nwarps, ncols1, ncols2, DKQ, DV, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, use_gfx1030_native, type_K, type_V>
                (Q_tmp, K_h2, V_h2, K_c, V_c, maskh, ne01, logit_softcap, slope, KQ, KV_tmp,
                stride_K2, stride_V2, stride_K_bytes, stride_V_bytes, stride_mask,
...
#pragma unroll
    for (int jc0 = 0; jc0 < cpw; ++jc0) {
        KQ_sum[jc0] = ggml_cuda_warp_reduce_sum_gfx1030<use_gfx1030_native, warp_size>(KQ_sum[jc0]);
    }
...
static void launch_fattn_tile_switch_ncols1(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const bool use_gfx1030_native = ggml_cuda_fattn_use_gfx1030_native();
    ...
    if constexpr (DKQ <= 128) {
        if (Q->ne[1] > 32/ncols2) {
            ...
            fattn_kernel_t fattn_kernel = use_gfx1030_native
                ? flash_attn_tile<DKQ, DV, cols_per_block/ncols2, ncols2, use_logit_softcap, true, type_K, type_V>
                : flash_attn_tile<DKQ, DV, cols_per_block/ncols2, ncols2, use_logit_softcap, false, type_K, type_V>;
            launch_fattn<DV, cols_per_block/ncols2, ncols2>
                (ctx, dst, fattn_kernel, nwarps, nbytes_shared, nbatch_fa,
                type_K == GGML_TYPE_F16, type_V == GGML_TYPE_F16, false, warp_size);
            return;
        }
    }
#endif // GGML_USE_HIP
...
#define DECL_FATTN_TILE_CASE_Q8_0(DKQ, DV)                                      \
    template void ggml_cuda_flash_attn_ext_tile_case                            \
    <DKQ, DV, GGML_TYPE_Q8_0, GGML_TYPE_Q8_0>(                                  \
        ggml_backend_cuda_context & ctx, ggml_tensor * dst)                     \

extern DECL_FATTN_TILE_CASE_Q8_0(256, 256);
