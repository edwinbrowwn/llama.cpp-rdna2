...
#if defined(GGML_USE_HIP)
// RDNA-optimized half2 kernel for the lightning indexer (N_EMBD=128, N_HEAD=64 or 32).
//
// The generic vector kernel reduces each (head, key) dot product over the full 32-lane
// warp, which makes it shuffle-bound: every key costs N_HEAD full warp reductions and Q
// is re-read from shared memory by all 32 lanes for every head. Here the warp is split
// into 8 groups of 4 lanes; each group owns one head per pass and reduces only over its
// own 4 lanes (2 shuffle steps instead of 5). Each lane keeps 32 of the 128 K dimensions
// in registers as half2 for K_VECS_PER_WARP keys, so K is read from VRAM exactly once,
// and the dot products run as HFMA2 (2 flops/lane/instr vs 1 for the fp32 kernel).
// Half-precision accumulation covers only the 32 dims of one lane before being widened
// to fp32, which is the same numerics class as the fp16-input WMMA path used on NVIDIA.
// Measured on a Radeon Pro V620 (gfx1030) at n_kv=65536, F16 K: ~3.2x the generic kernel.
template <int WARPS_PER_BLOCK, int K_VECS_PER_BLOCK, int64_t N_EMBD, int64_t N_HEAD, ggml_type TYPE_K>
static __global__ void __launch_bounds__(WARPS_PER_BLOCK*32, 1) lightning_indexer_kernel_vec_hip(
        const float * Q, const char * K, const float * W, const half * M, float * dst,
        int64_t n_stream, int64_t n_batch, int64_t n_kv,
        size_t nb1, size_t nb2, size_t nb3,
        size_t nbq1, size_t nbq2, size_t nbq3,
    ...
    const int head_sub = i_lane / LANES_PER_HEAD;   // head within the pass owned by this lane group
    const int dim_sub  = i_lane % LANES_PER_HEAD;   // 32-dim slice of the head owned by this lane
    ...
    // phase 1 - load (and dequantize if needed) K to registers as half2
    half2 k_reg[K_VECS_PER_WARP][H2_PER_LANE];

    if constexpr (TYPE_K == GGML_TYPE_F16) {
        // direct vectorized copy, 4 half2 at a time
#pragma unroll
        for (int k = 0; k < K_VECS_PER_WARP; ++k) {
...
#pragma unroll
    ...
    } else if constexpr (TYPE_K == GGML_TYPE_F32) {
#pragma unroll
        for (int k = 0; k < K_VECS_PER_WARP; ++k) {
            const int i_kv = start_kv + k;
#pragma unroll
            for (int j = 0; j < H2_PER_LANE; ++j) {
                k_reg[k][j] = __float2half2_rn(0.0f);
                if (i_kv < n_kv) {
                    const float2 * k_base = (const float2 *) ((const char *) K + i_kv*nbk2 + i_stream*nbk3);
                    const float2   kv     = k_base[dim_sub*H2_PER_LANE + j];
                    k_reg[k][j] = __floats2half2_rn(kv.x, kv.y);
                }
            }
        }
    } else if constexpr (TYPE_K == GGML_TYPE_BF16) {
        // bf16 dequantizes to float only; widen via float and repack to half2
        constexpr dequantize_V_t dequantize_k = get_dequantize_V<TYPE_K, float, 4>();
#pragma unroll
        for (int k = 0; k < K_VECS_PER_WARP; ++k) {
            const int i_kv = start_kv + k;
#pragma unroll
            for (int j = 0; j < H2_PER_LANE/2; ++j) {
                if (i_kv < n_kv) {
                    const void * k_base = (const void *) ((const char *) K + i_kv*nbk2 + i_stream*nbk3);
                    float tmp[4];
                    dequantize_k(k_base, tmp, (dim_sub*H2_PER_LANE + 2*j) * 2);
                    k_reg[k][2*j+0] = __floats2half2_rn(tmp[0], tmp[1]);
                    k_reg[k][2*j+1] = __floats2half2_rn(tmp[2], tmp[3]);
                } else {
...
#pragma unroll
    ...
    // phase 2/3 - iterate over heads in passes of HEADS_PER_PASS, Q double-buffered in shared mem
...
#pragma unroll
    ...
    for (int p = 0; p < N_PASS; ++p) {
        // stage the next pass while computing the current one; the __syncthreads() at
        // the end of the loop protects both buffers (a warp can never run more than one
        // pass ahead of the slowest warp in the block).
        if (p + 1 < N_PASS) {
            stage_q(p + 1);
        }
...
#pragma unroll
            ...
            // reduce over the LANES_PER_HEAD lanes that share this head
#pragma unroll
    ...
    // combine the HEADS_PER_PASS partial scores (held by lanes 0, 4, 8, ..., 28) on lane 0;
    // all other lanes hold zero so plain shuffles suffice
#pragma unroll
    for (int k = 0; k < K_VECS_PER_WARP; ++k) {
...
void ggml_cuda_lightning_indexer(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (rdna2_master_shape) {
        ...
        if (lightning_indexer_rdna2_subwave4_enabled()) {
            ...
            // Keep the generic master kernel as the differential reference;
            // the older RDNA2 half2 path remains the fallback outside this
            // exact production guard for other lightning-indexer users.
    ...
    if (n_embd == 128 && n_head == 64) {
        {
            // use RDNA-optimized half2 kernel
            constexpr int K_VECS_PER_BLOCK = 64;
            constexpr int WARPS_PER_BLOCK = 8;
            ...
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_F16)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_Q4_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_Q4_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_Q5_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_Q5_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_Q8_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_BF16)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 64, k, GGML_TYPE_F32)
            GGML_ABORT("fatal error");
        }
    } else if (n_embd == 128 && n_head == 32) {
        {
            ...
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_F16)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_Q4_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_Q4_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_Q5_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_Q5_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_Q8_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_BF16)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_vec_hip, 128, 32, k, GGML_TYPE_F32)
            GGML_ABORT("fatal error");
        }
    } else {
        GGML_ABORT("fatal error");
