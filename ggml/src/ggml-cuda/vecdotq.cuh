...
// Experimental gfx1030 path. It preserves the existing Q8_1 bytes and
// constructs the interleaved LO/HI digit words in registers.
template <int vdr> static __device__ __forceinline__ float vec_dot_q4_0_q8_1_dot8_impl(
    const uint32_t * v, const block_q8_1 * bq8_1, const block_q8_1_sum_hi * sum_hi,
    const int iqs, const float & d4, const half2 & ds8) {
...
#pragma unroll
    for (int i = 0; i < vdr; ++i) {
        ...
        const uint32_t qa0  = (uint32_t) get_int_b4(bq8_1->qs, iqs + i);
        const uint32_t qa1  = (uint32_t) get_int_b4(bq8_1->qs, iqs + i + QI4_0);
        ...
        acc_lo = ggml_hip_udot8_u4(qw_u, qa_lo, acc_lo);
        acc_hi = ggml_hip_sdot8_i4((int) qw_s, (int) qa_hi, acc_hi);
    }

    const int sumi = acc_lo + 16 * acc_hi + 128 * (int) sum_hi->sum_hi[iqs / VDR_Q4_0_Q8_1_MMVQ];
...
static __device__ __forceinline__ float vec_dot_q4_0_q8_1_dot8(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1,
    const block_q8_1_sum_hi * __restrict__ sum_hi, const int & kbx, const int & iqs) {
...
#pragma unroll
    ...
    return vec_dot_q4_0_q8_1_dot8_impl<VDR_Q4_0_Q8_1_MMVQ>(v, bq8_1, sum_hi, iqs, bq4_0->d, bq8_1->ds);
}
