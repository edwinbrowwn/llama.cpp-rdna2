...
llama_kv_cache::llama_kv_cache(
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        if (k) {
            ggml_format_name(k, "%s_k_l%d", name_prefix.c_str(), il);
        }
        if (v) {
            ggml_format_name(v, "%s_v_l%d", name_prefix.c_str(), il);
        }
        if (k_tail) {
            ggml_format_name(k_tail, "%s_k_tail_l%d", name_prefix.c_str(), il);
        }
        if (v_tail) {
            ggml_format_name(v_tail, "%s_v_tail_l%d", name_prefix.c_str(), il);
        }
